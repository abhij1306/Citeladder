...
async def _fetch_well_known(
    ctx: PhaseContext, url: str, *, purpose: str, max_bytes: int
) -> FetchResult | None:
    """Fetch one bounded well-known file; return ``None`` on failure."""
    ...
    except Exception:  # noqa: BLE001 - cancellation is a BaseException
        return None
...
async def collect_site_setup(
    ...
    *,
    ...
    if robots_policy is not None:
        declared_sitemaps = [str(url)[:2048] for url in robots_policy.sitemaps()[:16]]
...
async def _llms_facts(
    ctx: PhaseContext, authority: str, robots_policy: RobotsPolicy | None
    ...
    result = await _fetch_well_known(
        ctx,
        llms_url,
...
async def _fetch_sitemap_document(
    ctx: PhaseContext, fetcher: SecureFetcher, url: str
) -> FetchResult | None:
    ...
    async with ctx.session_factory() as session:
        ...
        if outcome.output is not None and outcome.result is not None:
            (
                ...
                admission,
                admitted_delta,
            ) = await _persist_discover_success(
                ctx,
                session,
                crawl=crawl,
                task=task,
        ...
        if succeeded_artifact_id is not None:
            ...
            should_retry, retry_attempt = _failure_retry_state(task, outcome)
...
def _candidates_for(
    output: DiscoveryOutput, depth: int, *, input_mode: str
