...
pytestmark = pytest.mark.skipif(
    ...
    reason="live browser-rung acceptance is opt-in (CITELADDER_LIVE_BROWSER_RUNG=1)",
)
...
async def test_off_host_request_from_a_live_page_is_aborted():
    """The route guard in isolation — the page itself tries to leave the host."""
    settings = _settings()
    target = await _resolve(JS_SHELL_URL, settings)
    transport = PatchrightTransport(settings=settings)
    ...
    try:
        ...
        async def _route(route, request):
            ...
            if not _same_site(request.url, target.host):
                await route.abort()
                return
