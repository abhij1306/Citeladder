...
@pytest.mark.asyncio
async def test_shopify_start_requires_shop(
    client: httpx.AsyncClient,
    db_session,
    _oauth_credentials: None,
    _fake_oauth: _FakeOAuthServer,
) -> None:
    await _register(client, "int-shopify-noshop@example.com")
    resp = await client.get(f"{_BASE}/oauth/shopify/start")
    assert resp.status_code == 422
    assert resp.json()["detail"] == "oauth_shop_invalid"


@pytest.mark.asyncio
async def test_shopify_start_rejects_hostile_shop(
    client: httpx.AsyncClient,
    db_session,
    _oauth_credentials: None,
    _fake_oauth: _FakeOAuthServer,
) -> None:
    await _register(client, "int-shopify-hostile@example.com")
    for hostile in (
        "shop.myshopify.com.evil.com",
        "myshopify.com",
        "a.b.myshopify.com",
        "https://volt-city.myshopify.com.evil.com/admin",
    ):
        resp = await client.get(
            f"{_BASE}/oauth/shopify/start", params={"shop": hostile}
        )
        assert resp.status_code == 422
        assert resp.json()["detail"] == "oauth_shop_invalid"
    # No state row was minted for a hostile target.
    count = await db_session.scalar(select(func.count(IntegrationOAuthState.jti)))
    assert count == 0


@pytest.mark.asyncio
async def test_non_shopify_start_rejects_shop_param(
    client: httpx.AsyncClient,
    db_session,
    _oauth_credentials: None,
    _fake_oauth: _FakeOAuthServer,
) -> None:
    await _register(client, "int-gsc-shop@example.com")
    resp = await client.get(
        f"{_BASE}/oauth/gsc/start", params={"shop": "volt-city.myshopify.com"}
    )
    assert resp.status_code == 422
    assert resp.json()["detail"] == "oauth_shop_invalid"
...
async def test_shopify_callback_bad_hmac_rejected_before_any_exchange(
    ...
    assert callback.status_code == 302
    assert callback.headers["location"] == _landing("error=oauth_state_invalid")
    # Nothing exchanged, nothing persisted; the state stays UNCONSUMED (the
    # HMAC check precedes consumption — a legitimate retry still works).
    assert _fake_oauth.shopify_token_calls() == []
    assert await _grants(db_session) == []
    state_row = (await db_session.execute(select(IntegrationOAuthState))).scalar_one()
    assert state_row.consumed_at is None

    # The same state with a VALID signature still completes.
    retry = await _shopify_callback(client, state)
    assert retry.headers["location"] == _landing("connected=shopify")
...
async def test_shopify_callback_shop_mismatch_rejected(
    ...
    callback = await _shopify_callback(client, state, shop="other-shop.myshopify.com")
    assert callback.status_code == 302
    assert callback.headers["location"] == _landing("error=oauth_state_invalid")
    assert _fake_oauth.shopify_token_calls() == []
    assert await _grants(db_session) == []
...
async def test_shopify_reconnect_repoints_single_connection(
    ...
    """One (workspace, shopify_oauth) grant: a re-connect re-points account_ref."""
    await _register(client, "int-shopify-reconnect@example.com")
    state = _state_from_start(await _start(client, "shopify", params={"shop": _SHOP}))
    first = await _shopify_callback(client, state)
    assert "connected=shopify" in first.headers["location"]

    state = _state_from_start(await _start(client, "shopify", params={"shop": _SHOP}))
    second = await _shopify_callback(client, state)
    assert "connected=shopify" in second.headers["location"]

    # Still ONE grant + ONE connection; the token rotated in place.
    grants = await _grants(db_session)
    assert len(grants) == 1
    (connection,) = await _connections(db_session)
    assert connection.account_ref == _SHOP
