...
# --- Shopify mappings: the "property" IS the connection's shop ---------------


async def _seed_shopify_connection(
    db_session, *, workspace_id: uuid.UUID, account_ref: str = "volt-city.myshopify.com"
) -> IntegrationConnection:
    grant = IntegrationOAuthGrant(
        workspace_id=workspace_id, transport="shopify_oauth", status="connected"
    )
    db_session.add(grant)
    await db_session.flush()
...
@pytest.mark.asyncio
async def test_create_shopify_mapping_canonicalizes_property_ref(
    client: httpx.AsyncClient, db_session
) -> None:
    await _register(client, "map-shopify@example.com")
    ws = await _workspace_id(db_session)
    shopify = await _seed_shopify_connection(db_session, workspace_id=ws)
    project = await _seed_project(db_session, workspace_id=ws)

    resp = await client.post(
        f"{_BASE}/{shopify.id}/mappings",
        json={
            "provider": "shopify",
            # A bare/uncanonical shop name canonicalizes onto the shop host.
            "property_ref": " Volt-City ",
            "project_id": str(project.id),
        },
    )
    assert resp.status_code == 201
    ...
    assert body["property_ref"] == "volt-city.myshopify.com"


@pytest.mark.asyncio
async def test_create_shopify_mapping_mismatched_shop_rejected_422(
    client: httpx.AsyncClient, db_session
) -> None:
    await _register(client, "map-shopify-mismatch@example.com")
    ws = await _workspace_id(db_session)
    shopify = await _seed_shopify_connection(db_session, workspace_id=ws)
    project = await _seed_project(db_session, workspace_id=ws)
    ...
    resp = await client.post(
        ...
        json={
            "provider": "shopify",
            # A different shop than the connection's account_ref.
            "property_ref": "other-shop.myshopify.com",
            "project_id": str(project.id),
        },
    )
    assert resp.status_code == 422
...
@pytest.mark.asyncio
async def test_create_shopify_mapping_hostile_shop_rejected_422(
    client: httpx.AsyncClient, db_session
) -> None:
    await _register(client, "map-shopify-hostile@example.com")
    ws = await _workspace_id(db_session)
    shopify = await _seed_shopify_connection(db_session, workspace_id=ws)
    project = await _seed_project(db_session, workspace_id=ws)
    ...
    resp = await client.post(
        ...
        json={
            "provider": "shopify",
            "property_ref": "volt-city.myshopify.com.evil.com",
            "project_id": str(project.id),
        },
    )
    assert resp.status_code == 422
