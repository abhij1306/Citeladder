...
describe('CatalogTable rows', () => {
  it('links the product name to the evidence drill-down and shows the variant', () => {
    ...
    expect(screen.getByRole('columnheader', { name: 'Variants' })).toBeInTheDocument();
    expect(screen.queryByRole('columnheader', { name: 'Origin' })).not.toBeInTheDocument();
    expect(screen.queryByRole('columnheader', { name: 'Feed health' })).not.toBeInTheDocument();
  ...
  it('shows the persisted CSV variant_count instead of parsing variants', () => {
    renderTable([makeProduct(1, { attributes: { brand: 'Acme', variant_count: '5' } })]);
