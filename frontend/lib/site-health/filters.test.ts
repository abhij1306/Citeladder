...
describe('inventory filter serialization (cursor round-trip)', () => {
  it('round-trips a full filter set through URL query state', () => {
    ...
    expect(params.get('page_kind')).toBe('article');
  ...
  it('omits the empty page_kind default', () => {
    const params = serializeInventoryFilters(emptyInventoryFilters);
    expect(params.get('page_kind')).toBeNull();
    expect(parseInventoryFilters(new URLSearchParams()).page_kind).toBe('');
  });
...
describe('toInventoryParams', () => {
  it('maps filters + cursor to request params (monitored tri-state)', () => {
    const params = toInventoryParams(
      { query: ' foo ', status: 'ok', monitored: 'unmonitored', page_kind: 'pricing' },
      'C1',
      200,
    );
  ...
  it('drops empty query/status/page_kind and "all" monitored to undefined', () => {
    const params = toInventoryParams(emptyInventoryFilters);
    expect(params.query).toBeUndefined();
    expect(params.status).toBeUndefined();
    expect(params.monitored).toBeUndefined();
    expect(params.page_kind).toBeUndefined();
...
describe('changeInventoryFilters drops the cursor', () => {
  it('always resets cursor to null on a filter change', () => {
    const current: InventoryFilters = { query: 'a', status: '', monitored: 'all', page_kind: '' };
    const { filters, cursor } = changeInventoryFilters(current, { query: 'b' });
    expect(cursor).toBeNull();
    expect(filters.query).toBe('b');
  });

  it('resets the cursor when only the page kind changes', () => {
    const { filters, cursor } = changeInventoryFilters(emptyInventoryFilters, {
      page_kind: 'docs',
    });
    expect(cursor).toBeNull();
    expect(filters.page_kind).toBe('docs');
  });
});
...
describe('inventoryFiltersEqual (cursor validity check)', () => {
  it('treats trimmed-equal queries as equal', () => {
    expect(
      inventoryFiltersEqual(
        { query: 'foo', status: '', monitored: 'all', page_kind: '' },
        { query: '  foo  ', status: '', monitored: 'all', page_kind: '' },
      ),
    ).toBe(true);
  });
  ...
  it('detects a differing filter', () => {
    expect(
      inventoryFiltersEqual(
        { query: 'foo', status: '', monitored: 'all', page_kind: '' },
        { query: 'foo', status: '', monitored: 'monitored', page_kind: '' },
      ),
    ).toBe(false);
  });

  it('detects a differing page kind', () => {
    expect(
      inventoryFiltersEqual(
        { ...emptyInventoryFilters, page_kind: 'article' },
        { ...emptyInventoryFilters, page_kind: 'docs' },
      ),
    ).toBe(false);
  });
});
...
describe('issue filter serialization', () => {
  ...
  it('maps the page_kind filter onto the wire param', () => {
    const params = toIssueParams({ ...emptyIssueFilters, page_kind: 'faq' });
  ...
  it('changeIssueFilters drops the cursor and issueFiltersEqual compares', () => {
    ...
    expect(issueFiltersEqual(emptyIssueFilters, { ...emptyIssueFilters, page_kind: 'faq' })).toBe(
      false,
    );
  });
});
