# CiteLadder — AI-Visibility Product Documentation (Summary)

## Problem & goals
CiteLadder is a greenfield AEO / AI-visibility SaaS (an original "Searchable"-class product).
...
2. **Code the MVP** — build CiteLadder's **AI-visibility benchmarking slice** on the **target
...
- **Implemented as CiteLadder's own `ai_visibility` domain** (`backend/app/` with `models/`, `schemas/`,
...
- **Backend conventions**: FastAPI app-factory + router-per-
  ...
  **Next.js App Router** (CiteLadder's Vercel target), not Vite.
- **CiteLadder does NOT include** a crawler/acquisition/browser engine, and never uses integer-PK/
