...
- Implemented: locked cannibalization, property-relative CTR-gap, emerging, and declining semantics; available/partial/unavailable/insufficient-history states; exact thresholds and positive/boundary fixtures; distinct Opportunity rules; labeled Search Demand rows and honest state notes. Complexity-driven seams keep projection, detector source, strict algorithms, and Opportunity mapping within repository limits. Agent-review follow-up added evidence-only override cache invalidation, complete 28-day trend coverage, pre-materialization SQL bounds, snapshot-bound cursors, bounded resolver batches, citation-safe grounding budgets, exact classifier provenance, and conflict-safe projection inserts.
...
- Commit: wave commit
- Cumulative changed files vs wave merge base: 62
...
#### E01 — completed 2026-08-15
- Wave / branch: W5 / `feat/aeo-wave-5-site-intelligence`
- Owners: Site Health's persisted current rule evaluations, config-owned presentation taxonomy, project API, and Website tab surface.
- Implemented: exactly 20 explicit rule mappings across the seven locked dimensions; exact crawl/analyzer/extractor selection; pass/fail/not-applicable/error plus expected/observed coverage; fail-first bounded evaluation links; and an evidence-linked Website ledger with no composite score.
...
- Commit: wave commit
- Cumulative changed files vs wave merge base: 47
- Local verification: pure and API reconciliation pytest 5 passed; combined Website/API Vitest 66 passed; both named Playwright browser proofs passed on `/site`; Ruff, mypy, ESLint, design policy, and complexity gates passed. Sanitized live read-only SQL reconciled 21 dimension rows across 150-, 114-, and 22-analysis corpora with exact expected totals. Agent-review wave close passed 47 focused backend and 67 focused frontend tests; both browser proofs and the production build passed again. Added boundaries for indexable Opportunity targets, canonical unresolved edges, analyze-population coverage, analyzer-version queue slots, task/crawl workspace matching, complete readiness reconciliation, explicit source suggestions, and snapshot-bound node/edge cursor traversal. The singular migration, including the constrained evaluation-outcome vocabulary, upgraded a fresh disposable database with zero ORM drift.
- Evaluation artifact: `docs/evaluations/2026-08-15-w5-site-intelligence.md`
- Advanced to internal slice: wave close
